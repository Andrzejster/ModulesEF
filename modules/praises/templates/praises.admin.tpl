<h3 class="ui-corner-all">{$SystemVersion} - {i18n('Praises')}</h3>
{if $message}<div class="{$class}">{$message}</div>{/if}

<script>var addr_modules = '{$ADDR_MODULES}';</script>
<script src="{$ADDR_MODULES}praises/templates/javascripts/users.js"></script>
...
{if $user}
	<form id="This" action="{$URL_REQUEST}" method="post">
		...
		{/if}
		<div class="tbl2">
			<div class="formLabel sep_1 grid_3"><label for="Praises">{i18n('Reason')}:</label></div>
			<div class="formField grid_7"><textarea name="praises" id="Praises" rows="3" class="resize">{$praises}</textarea></div>
		</div>
		{if ! $edit}
			<div class="tbl1">
